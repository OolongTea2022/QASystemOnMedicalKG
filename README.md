...
这个项目是基于刘焕勇老师的医疗知识图谱以及问答系统的基础上进行学习和改造的
...
# 主要改动
1)  更改爬取过程 ( prepare_data/crawl_drug.py 和 prepare_data/data_spider)

原项目所爬取的医疗网站 ( 寻医问药 [https://jib.xywy.com](https://jib.xywy.com/) ）中大部分疾病所对应药物信息都有所缺失，而另外一个医疗相关的网站 ( 39健康网 [https://jbk.39.net/](https://jbk.39.net/) ) 的信息较全，但是整体结构以及爬取过程与寻医问药不同，如果完全采用寻医问药则爬取部分基本都要改。
所以我选择在原有的爬取寻医问药的过程基础上，只爬取对应疾病在39健康网中的药物来拼成较为完整的数据。

具体操作也不难，我们只需要将之前爬取的疾病名称转换为拼音首字母，即可在 [https://jbk.39.net/{疾病拼音首字母}/]() 找到对应疾病的详情页，
然后再爬取对应药品信息，以补充原来数据中缺失的药品信息 （drug_detail）。虽然并不是所有的疾病详情页都能找到，但大部分都可以找到。具体细节可以看 crawl_drug.py 


2) 建立简单的后端以及交互界面 ( app.py 和 static/index.html )

在配置好环境以及数据后运行 app.py 再访问 位于 static 的 index.html 即可
